...
- Play tricks. First player unable to follow suit MUST reveal a trump
  card from hand (their choice) -> trump suit locked immediately.
- The instant trump is revealed, ALL remaining boot cards (32, 8 per
  seat) are dealt out immediately, even mid-trick. The trick already in
  progress simply continues to completion with players now holding
  their full enlarged hands.
...
class MendikotHand:
    ...
    def reveal_trump(self, seat: int, card: str) -> dict:
        """Player reveals a trump card from hand (their own choice) when void in led suit."""
        ...
        # This seat's void-in-led-suit status is now locked in for the rest of
        # THIS trick: even though they're about to receive new boot cards that
        # might include the led suit, they remain exempt from follow-suit for
        # their play in this specific trick (cleared when the next trick starts).
        self.trick_void_exempt_seats.add(seat)

        # Deal all remaining boot cards to everyone immediately, mid-trick.
        # The current trick (if any cards already played this trick) simply
        # continues with players now holding their full enlarged hands; the
        # revealed card stays in the revealer's hand and is now playable.
        self._deal_phase2()
    ...
    def _resolve_trick(self) -> dict:
        ...
        if self.phase == Phase.PHASE1_PLAY and self.tricks_played == 5:
            # Reaching here means trump was NEVER revealed during the first 5
            # tricks (if it had been, reveal_trump() already dealt the boot
            # and flipped phase to PHASE2_PLAY, so this branch wouldn't be
            # reached with phase still PHASE1_PLAY).
            trick_summary["no_trump_locked"] = True
            self._deal_phase2()
            self.phase = Phase.PHASE2_PLAY
            trick_summary["phase2_dealt"] = True
        elif self.tricks_played == 13:
            self._finalize_hand()
        ...
        while True:
            ...
            # ---------------- REVEAL TRUMP ----------------
                ...
                await broadcast(room, {
                    ...
                    "phase2_dealt": ev.get("phase2_dealt", False),
                })
                await send_hand_state_to_all(room)
...
<div id="app">
  ...
  <div id="view-home" class="home-wrap">
    <h1 class="home-title">ꯗꯥꯁ ꯐꯥꯕꯤ<span class="stamp-suits">♠ ♥ ♦ ♣</span></h1>
...
(function() {
  ...
  function handleMessage(msg) {
    switch (msg.type) {
      ...
      case 'card_played':
        // visual feedback handled via next game_state render
        break;
