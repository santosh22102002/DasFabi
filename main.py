...
<style>
  :root {
    ...
    --card-width: clamp(36px, 10vw, 64px);
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .app { height: 100dvh; display: flex; flex-direction: column; background: radial-gradient(ellipse at center, var(--felt-green) 0%, var(--felt-green-dark) 100%); }
  .view { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px; animation: fadeIn 0.4s ease; }
  .menu-title { font-size: clamp(1.8rem, 7vw, 3rem); font-weight: 800; color: var(--accent-gold); text-shadow: 0 2px 10px rgba(0,0,0,0.5); margin-bottom: 6px; letter-spacing: 2px; }
  .menu-sub { color: rgba(255,255,255,0.6); margin-bottom: 32px; font-size: 0.9rem; }
  .menu-btn {
  ...
  .form-title { font-size: 1.3rem; font-weight: 700; margin-bottom: 16px; text-align: center; color: var(--accent-gold); }
  .input-group { margin-bottom: 14px; }
  .input-group label { display: block; margin-bottom: 5px; font-size: 0.8rem; color: rgba(255,255,255,0.7); font-weight: 500; }
  ...
  .team-select { display: flex; gap: 8px; margin-bottom: 14px; }
  .team-option {
    flex: 1;
    padding: 10px;
    border-radius: var(--radius-sm);
  ...
  .form-actions button { flex: 1; padding: 10px; border-radius: var(--radius-sm); border: none; font-weight: 700; cursor: pointer; font-size: 0.9rem; }
  .btn-back { background: rgba(255,255,255,0.08); color: #fff; }
  .btn-submit { background: var(--accent-gold); color: #1a1a1a; }
  .room-code { font-size: 1.8rem; font-weight: 800; letter-spacing: 3px; color: var(--accent-gold); text-align: center; margin: 8px 0; }
  .seats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; width: min(300px, 82vw); margin: 16px 0; }
  .seat {
    background: rgba(0,0,0,0.25);
    border: 2px solid rgba(255,255,255,0.1);
  ...
  .seat-team { font-size: 0.7rem; opacity: 0.6; margin-top: 3px; }
  .seat-status { font-size: 0.7rem; margin-top: 5px; padding: 2px 6px; border-radius: 8px; display: inline-block; }
  ...
  .game-view { flex: 1; display: flex; flex-direction: column; position: relative; overflow: hidden; min-height: 0; }
  .score-cluster {
    display: flex;
    align-items: center;
  ...
  .score-label { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 1px; opacity: 0.7; margin-bottom: 1px; }
  .score-number { font-size: 1.2rem; font-weight: 800; }
  .score-team-a { color: #6bb5ff; }
  .score-team-b { color: #ff8a8a; }
  ...
  .trump-label { font-size: 0.6rem; text-transform: uppercase; letter-spacing: 1px; opacity: 0.6; }
  .trump-card-mini {
  ...
  .seat-marker.top { top: 1%; left: 50%; transform: translateX(-50%); }
  .seat-marker.left { left: 1%; top: 50%; transform: translateY(-50%); }
  .seat-marker.right { right: 1%; top: 50%; transform: translateY(-50%); }
  .seat-marker.bottom { bottom: 1%; left: 50%; transform: translateX(-50%); }
  .bot-badge { font-size: 0.55rem; background: rgba(255,255,255,0.1); padding: 1px 5px; border-radius: 5px; margin-top: 2px; }
  .trick-center {
    position: relative;
  ...
  .card:hover:not(.disabled):not(.back) { transform: translateY(-4px); box-shadow: 0 4px 14px rgba(0,0,0,0.4); }
  ...
  .hand-card-wrapper { transition: margin 0.3s ease; flex-shrink: 0; }
  .trump-flash-overlay {
    position: fixed;
    inset: 0;
    display: flex;
  ...
  .trump-flash-suit { font-size: clamp(3.5rem, 12vw, 6rem); animation: suitPulse 0.6s ease infinite; }
  .trump-flash-text { font-size: clamp(1rem, 3.5vw, 1.5rem); font-weight: 800; color: var(--accent-gold); margin-top: 8px; }
  .trump-flash-sub { font-size: 0.85rem; opacity: 0.8; margin-top: 4px; }
  .reveal-btn {
    position: absolute;
  ...
  .result-title { font-size: 1.4rem; font-weight: 800; color: var(--accent-gold); margin-bottom: 12px; }
  .result-scores { display: flex; justify-content: center; gap: 24px; margin: 12px 0; }
  .result-team { text-align: center; }
  .result-team-name { font-size: 0.75rem; text-transform: uppercase; opacity: 0.7; }
  .result-team-score { font-size: 1.8rem; font-weight: 800; margin-top: 3px; }
  .result-mendi { display: flex; gap: 3px; justify-content: center; margin-top: 6px; flex-wrap: wrap; }
  .result-btn {
    margin-top: 16px;
    padding: 10px 28px;
  ...
  .confirm-text { margin-bottom: 16px; font-size: 0.95rem; }
  .confirm-buttons { display: flex; gap: 8px; }
  .confirm-buttons button { flex: 1; padding: 8px; border-radius: var(--radius-sm); border: none; font-weight: 600; cursor: pointer; font-size: 0.9rem; }
  .btn-cancel { background: rgba(255,255,255,0.1); color: #fff; }
  .btn-confirm { background: #c41e3a; color: #fff; }
  @media (min-width: 600px) {
    :root { --card-width: 58px; --seat-size: 52px; }
    .seat-marker.left { left: 6%; }
    .seat-marker.right { right: 6%; }
  }
  @media (min-width: 900px) {
    :root { --card-width: 64px; --seat-size: 56px; }
  }
  @media (max-height: 500px) {
    :root { --card-width: 34px; }
    .score-cluster { min-height: 44px; padding: 4px 8px; }
    .hand-strip { min-height: calc(var(--card-height) + 8px); padding-bottom: 6px; }
    .seat-avatar { width: 34px; height: 34px; font-size: 0.9rem; }
    .seat-name-tag { font-size: 0.6rem; max-width: 70px; }
    .trick-center { width: min(140px, 40vw); height: min(140px, 40vw); }
  }
  @media (max-height: 700px) and (min-width: 700px) {
    :root { --card-width: 44px; }
  }
</style>
</head>
<body>
      ...
      <div className="result-card">
        ...
        <div className="result-scores">
          <div className="result-team">
            <div className="result-team-name">Team A</div>
            <div className="result-team-score score-team-a">{scores.A}</div>
            <div className="result-mendi">
              {mendi.A.map((c,i) => <div key={i} className="mendi-mini" style={{color: SUIT_COLORS[c.suit]==='red'?'#c41e3a':'#1a1a1a'}}>{c.suit}</div>)}
            </div>
          </div>
          <div className="result-team">
            <div className="result-team-name">Team B</div>
            <div className="result-team-score score-team-b">{scores.B}</div>
            <div className="result-mendi">
              {mendi.B.map((c,i) => <div key={i} className="mendi-mini" style={{color: SUIT_COLORS[c.suit]==='red'?'#c41e3a':'#1a1a1a'}}>{c.suit}</div>)}
            </div>
          </div>
        </div>
...
function GameView({ gameState, trick, seat, onPlayCard, onRevealTrump, onRematch, onExit, showTrumpFlash, flashSuit, onTrumpDone, paused, handComplete, result, toasts, confirmExit, onConfirmExit, onCancelExit }) {
  ...
  const overlap = handCount > 1 ? -(Math.max(8, Math.min(32, (handCount - 1) * 5))) : 0;
  const seatPositions = ['bottom','left','top','right'];

  // Compute if player must reveal trump
  const ledSuit = trick.length > 0 ? trick[0].card.suit : null;
  const hasLedSuit = ledSuit ? myHand.some(c => c.suit === ledSuit) : true;
  const mustReveal = gameState?.turn === seat && !paused && !handComplete
    && gameState?.phase === 'phase1' && !gameState?.trump_suit
...
function App() {
  ...
  const handleMessage = useCallback((msg) => {
    ...
    if (type === 'room_created') {
      ...
      setGameState(prev => prev ? {...prev, mendi: msg.mendi, scores: msg.scores} : prev);
      ...
      setGameState(prev => prev ? {...prev, mendi: msg.mendi, scores: msg.scores, hand_complete: true, result: msg.result} : prev);
  ...
  // Keep handlerRef always pointing to latest handleMessage
  useEffect(() => {
