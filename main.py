...
class MendikotHand:
    ...
    def reveal_trump(self, seat: int, card: str) -> dict:
        ...
        trick, and (3) THEN deals all remaining boot cards to everyone.
        """
        ...
        # Step 1: lock the trump suit, and flip phase now (before any trick
        # resolution below) so _resolve_trick()'s no-trump-at-trick-5 branch
        # never fires here - that branch is only for the case where trump was
        # NEVER revealed, which is not what's happening in this method.
        ...
        # Step 3: deal all remaining boot cards to everyone, now that the
        # revealed card has already been played. If the trick just completed
        # above, _resolve_trick() already reset current_trick/led_suit/turn -
        # the deal below simply adds cards on top of that fresh state.
        # (Guard: reveal_trump only ever fires while phase was PHASE1_PLAY,
        # meaning tricks_played was < 5 at entry, so completing this trick
        # can reach at most trick 5 - never trick 13/HAND_COMPLETE. Guard
        # here anyway in case that invariant ever changes.)
        if self.phase != Phase.HAND_COMPLETE:
            self._deal_phase2()
            self.phase = Phase.PHASE2_PLAY
            result["phase2_dealt"] = True
