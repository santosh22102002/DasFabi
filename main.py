...
class MendikotHand:
    def __init__(self, dealer_seat: int = 0, rng: Optional[random.Random] = None):
        ...
        self.cards_won = {0: 0, 1: 0, 2: 0, 3: 0}  # total cards collected per seat
    ...
    def _finalize_hand(self):
        ...
        if team_mendi["A"] == team_mendi["B"]:
            # Tie-breaker: team with more total cards collected wins
...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* directional throw animations */
  .trick-slot.pos-bottom .pcard {
    animation: throwUp 0.35s cubic-bezier(.2,1.4,.4,1);
  }
  @keyframes throwUp {
    0% { transform: translateY(30px); opacity: 0; }
    100% { transform: translateY(0); opacity: 1; }
  }
  .trick-slot.pos-top .pcard {
    animation: throwDown 0.35s cubic-bezier(.2,1.4,.4,1);
  }
  @keyframes throwDown {
    0% { transform: translateY(-30px); opacity: 0; }
    100% { transform: translateY(0); opacity: 1; }
  }
  .trick-slot.pos-left .pcard {
    animation: throwRight 0.35s cubic-bezier(.2,1.4,.4,1);
  }
  @keyframes throwRight {
    0% { transform: translateX(-30px); opacity: 0; }
    100% { transform: translateX(0); opacity: 1; }
  }
  .trick-slot.pos-right .pcard {
    animation: throwLeft 0.35s cubic-bezier(.2,1.4,.4,1);
  }
  @keyframes throwLeft {
    0% { transform: translateX(30px); opacity: 0; }
    100% { transform: translateX(0); opacity: 1; }
  ...
  /* card collection sweep toward winner */
  .trick-center.collect-left .pcard {
    animation: sweepLeft 0.7s cubic-bezier(.4,0,.2,1) forwards;
  }
  @keyframes sweepLeft {
    0% { transform: translate(0, 0) scale(1); opacity: 1; }
    60% { opacity: 1; }
    100% { transform: translate(-90px, -80px) scale(0.5); opacity: 0; }
  }
  .trick-center.collect-right .pcard {
    animation: sweepRight 0.7s cubic-bezier(.4,0,.2,1) forwards;
  }
...
(function() {
  ...
  function showResultOverlay(winnerTeam, finalMendi) {
    ...
    const finalCards = S.gameState && S.gameState.final_cards;
    const myCards = finalCards ? finalCards[myTeam] : 0;
    const oppCards = finalCards ? finalCards[myTeam === 'A' ? 'B' : 'A'] : 0;
    const finalCards = S.gameState && S.gameState.final_cards;
    const myCards = finalCards ? finalCards[myTeam] : 0;
    const oppCards = finalCards ? finalCards[myTeam === 'A' ? 'B' : 'A'] : 0;
    if (winnerTeam === 'DRAW') {
      ...
      sub.textContent = '2 – 2 split • ' + myCards + '–' + oppCards + ' cards';
    } else if (winnerTeam === myTeam) {
      headline.textContent = 'You Win!';
      headline.className = 'result-headline';
      if (mine === theirs) {
        sub.textContent = mine + ' – ' + theirs + ' • Won on cards (' + myCards + '–' + oppCards + ')';
      } else {
      ...
      headline.className = 'result-headline draw';
      if (mine === theirs) {
        sub.textContent = theirs + ' – ' + mine + ' • Lost on cards (' + oppCards + '–' + myCards + ')';
      } else {
    ...
    row.innerHTML =
      '<div class="mendi-counter mine"><div class="mendi-label">Your Team</div><div class="ten-slots" id="result-my-slots"></div><div class="mendi-label" style="margin-top:4px">Cards: ' + myCards + '</div></div>' +
      '<div class="mendi-counter theirs"><div class="mendi-label">Opponents</div><div class="ten-slots" id="result-opp-slots"></div><div class="mendi-label" style="margin-top:4px">Cards: ' + oppCards + '</div></div>';
